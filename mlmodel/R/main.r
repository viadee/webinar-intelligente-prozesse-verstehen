...
features <- c('TypeClass', 'Year', 'Doors', 'Passengers', 'Costs', 'RepareType')
labels <- 'Rejected'
...
titanic_rf <- h2o.randomForest(x=features,
                       ...
                       nfolds = 10)
claim_automl = h2o.automl(x=features,
           y=labels,
           training_frame = h2o_train,
           max_models = 20,
           nfolds = 10)

lb <- claim_automl@leaderboard
print(lb, n = nrow(lb)) 

titanic_rf = claim_automl$leader
claim_automl$leader
